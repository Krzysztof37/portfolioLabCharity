...
public class InstitutionController {
    ...
    public String getInstitutionsList(Model model, HttpServletRequest request){

        List<Institution> institutionsList = (List<Institution>) institutionService.allInstitutions();
        PagedListHolder pagedListHolder = new PagedListHolder(institutionsList);
        int page = ServletRequestUtils.getIntParameter(request, "p", 0);
        pagedListHolder.setPage(page);
        pagedListHolder.setPageSize(3);
        model.addAttribute("institutionsList", pagedListHolder);
